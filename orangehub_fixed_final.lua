...
-- [ TỐI ƯU MÁY YẾU - TỰ ĐỘNG BẬT KHI KHỞI ĐỘNG ]
pcall(function()
...
getgenv().TweenSpeedFar  = 250
getgenv().TweenSpeedNear = 250
...
_tp = function(I)
    ...
    local speed = dist <= 15
        and (getgenv().TweenSpeedNear or 250)
        or  (getgenv().TweenSpeedFar  or 250)
...
-- Noryx logo: code-drawn and attached to the single minimize button.
do
    local b = nil
    if typeof(MinimizeButton) == "Instance" and MinimizeButton:IsA("GuiButton") then
        b = MinimizeButton
    elseif type(MinimizeButton) == "table" then
        for _,v in pairs(MinimizeButton) do if typeof(v)=="Instance" and v:IsA("GuiButton") then b=v break end end
    end
    if b then
        for _,v in ipairs(b:GetChildren()) do if v:IsA("ImageLabel") or v:IsA("ImageButton") then v.Visible=false end end
        local logo=Instance.new("Frame")
        logo.Name="NoryxCodeLogo"; logo.Size=UDim2.fromScale(1,1); logo.BackgroundTransparency=1; logo.ZIndex=50; logo.Parent=b
        local circle=Instance.new("Frame")
        circle.AnchorPoint=Vector2.new(.5,.5); circle.Position=UDim2.fromScale(.5,.5); circle.Size=UDim2.fromScale(.88,.88); circle.BackgroundColor3=Color3.fromRGB(235,18,18); circle.BorderSizePixel=0; circle.ZIndex=51; circle.Parent=logo
        local cc=Instance.new("UICorner"); cc.CornerRadius=UDim.new(1,0); cc.Parent=circle
        local cs=Instance.new("UIStroke"); cs.Color=Color3.fromRGB(5,5,5); cs.Thickness=4; cs.Parent=circle
        local n=Instance.new("TextLabel"); n.BackgroundTransparency=1; n.Size=UDim2.fromScale(.55,.65); n.Position=UDim2.fromScale(.06,.17); n.Font=Enum.Font.GothamBlack; n.Text="N"; n.TextScaled=true; n.TextColor3=Color3.new(1,1,1); n.TextStrokeColor3=Color3.new(0,0,0); n.TextStrokeTransparency=0; n.ZIndex=55; n.Parent=logo
        local x=Instance.new("TextLabel"); x.BackgroundTransparency=1; x.Size=UDim2.fromScale(.58,.65); x.Position=UDim2.fromScale(.39,.18); x.Font=Enum.Font.GothamBlack; x.Text="X"; x.TextScaled=true; x.TextColor3=Color3.new(1,1,1); x.TextStrokeColor3=Color3.new(0,0,0); x.TextStrokeTransparency=0; x.ZIndex=56; x.Parent=logo
    end
end
...
Tabs.VS:AddButton({
    ...
    Callback = function()
        ...
        local ToolIconID = "rbxassetid://109508733644147" 
...
Tabs.VS:AddButton({
    ...
    Callback = function()
        task.spawn(function()
            ...
            local objs = g:GetObjects("rbxassetid://109508733644147")
...
getgenv().PMT_GunFarm_Delay = getgenv().PMT_GunFarm_Delay or 0.05
...
task.spawn(function()
    ...
    -- Delay giữa các lần quét (tối ưu máy yếu)
    local scanDelay = 0.05  -- 1/20 giây
